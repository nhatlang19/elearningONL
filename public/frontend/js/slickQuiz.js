    ...
    $.slickQuiz = function(element, options) {
        var plugin   = this,
            ...
            defaults = {
                ...
                saveText: 'Lưu',
                nextQuestionText: 'Kế tiếp &raquo;',
                backButtonText: '&laquo; Trở lại',
                tryAgainText: '',
            ...
            $quiz 				   = $('.quiz'),
            $quizAnswerUser 	   = $('.quiz-answer-user'),
            $hidden 			   = $('#hidden'),
            $countDown			   = $('#countDown')
        ;
        ...
        plugin.method = {
            ...
            setupQuiz: function() {
                ...
                for (i in questions) {
                    if (questions.hasOwnProperty(i)) {
                        ...
                        // save Button
                        questionHTML.append('<a href="#" class="btn btn-primary ' + saveClass + '">' + plugin.config.saveText + '</a>');
                        
                        // If we're not showing responses per question, show next question button and make it check the answer too
                        if (!plugin.config.perQuestionResponseMessaging) {
                            questionHTML.append('<a href="#" class="btn btn-primary ' + nextQuestionClass + ' ' + checkAnswerClass + '">' + plugin.config.nextQuestionText + '</a>');
                        } else {
            ...
            balancedColumn: function() {
//            	if($quiz.height() > $quizAnswerUser.height() ) {
//            		$quizAnswerUser.height($quiz.height());
//            	}
            },
            
            // Load All Answer of User
            loadQuizAnswerUser: function() {
            	var html = '';
            	html +=	'<table class="table table-bordered">';
            	var length = quizValues.questions.length;
            	html += '<tr>';
            ...
            countDown: function() {
                ...
                if( typeof periods !== 'undefined') {
                	...
                	if(parseInt(periodArray[4])) {
                		shortly.setHours(shortly.getHours() + parseInt(periodArray[4]));
                	}
                	if(parseInt(periodArray[5])) {
                		shortly.setMinutes(shortly.getMinutes() + parseInt(periodArray[5]));
                	} 
                	if(parseInt(periodArray[5])) {
                		shortly.setSeconds(shortly.getSeconds() + parseInt(periodArray[6]));
                	}
                	...
                	//var shortly = new Date(); 
                    // set time 
            	    //shortly.setMinuteSeconds(shortly.getMinutes() + _TIME); 
                	var shortly = '+' + _TIME + 'm';
                }
                
                $hidden.countdown({until: shortly, format: 'HMS', onTick: countDown, onExpiry: liftOff}); 
        		 
        		function countDown(periods) { 
        		    $countDown.text(twoDigits(periods[4]) + ':' + twoDigits(periods[5]) + ':' + twoDigits(periods[6])); 
        		    if(periods[4] || periods[5] || periods[6]) {
        		    	if(!_LOGOUT) {
        		    		$.cookie(_CNAME, periods, { path: '/' });
        		    	}
        		    } else {
            ...
            displayQuestion: function(index) {
            	...
            	var currentQuestion = $($(_question).parent()[0]),
            		jumpQuestion = currentQuestion.children().eq(index),
	            ...
	            $(_question).hide();
	            if(index) {
	            	jumpQuestion.find(_prevQuestionBtn).show().end().fadeIn(500);
	            } else {
	            	jumpQuestion.find(_prevQuestionBtn).hide().end().fadeIn(500);
	            }
                // disable next button if it is last question.
                var tmp    = jumpQuestion.next(_question);
                if (!tmp.length) {
                	jumpQuestion.find(_nextQuestionBtn).hide();
                	jumpQuestion.find(_saveBtn).show();
                }
            },
            ...
            nextQuestion: function(nextButton) {
                ...
                if (nextQuestion.length) {
                    ...
                    // disable next button if it is last question.
                    var tmp    = nextQuestion.next(_question);
                    if (!tmp.length) {
                    	nextQuestion.find(_nextQuestionBtn).hide();
                    	nextQuestion.find(_saveBtn).show();
                    }
                } else {
        ...
        plugin.init = function() {
            ...
            plugin.method.loadQuizAnswerUser();
...
//                }, 510);
            ...
            // Bind "save" buttons
            $(_element + ' ' + _saveBtn).on('click', function(e) {
            	e.preventDefault();
            	
            	var val = confirm("Bạn chắc chắn muốn nộp bài?");
            	if(val) {
            		// remove cookie
            		$.removeCookie(_CNAME, { path: '/' });
            ...
            $('.answer-user table tr td').on('click', function(e) {
            	e.preventDefault();
            	var className = $(this).attr('class');
            	var index = className.replace('td-question', '');
            	plugin.method.displayQuestion(index);
