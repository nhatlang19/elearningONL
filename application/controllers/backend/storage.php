...
class Storage extends Ext_Controller {
	...
	function _loadContent() {
		...
		$base_url = base_url() . BACK_END_TMPL_PATH . 'storage/lists';
		$config = $this->configPagination($base_url
		, $this->storage_model->table_record_count
		, $per_page, self::_URI_SEGMENT);
		$this->pagination->initialize($config);
		$data['pagination'] = $this->pagination;

		return $this->load->view(BACK_END_TMPL_PATH . 'storage/lists', $data, TRUE);
	}
	...
	function export($id) {
		...
		$this->wordcomponent->exportStorages($filename, $storages);
	}

	function _saveFileData($storage_id, $fileName) {
	...
	function uploadfile($storage_id = 0) {
