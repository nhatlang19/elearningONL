...
if (! class_exists ( 'Commonobj' )) {
	class Commonobj {
		...
		/**
		 * Hàm tính điểm
		 * @param int $total_score số điểm tối đa ( 10 )
		 * @param int $number_question tổng số câu hỏi
		 * @param int $number_correct số câu trả lời đúng
		...
		public function set_forceadd($set, $item) { //echo $set."---".$item; die();
			$items = explode ( ',', $set );
			if (in_array ( $item, $items )) {
				return $set;
			} else {
		...
		function upload_file($file_upload, $new_filename = '', $file_types, $max_size, $filename_valid_chars, $destination, &$message) {
			if ($_FILES [$file_upload] ['name']) {
				if (! in_array ( $_FILES [$file_upload] ["type"], $file_types )) {
					...
					return '';
				} else if (($_FILES [$file_upload] ["size"] > $max_size)) {
