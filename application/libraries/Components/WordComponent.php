...
class WordComponent {
		...
		foreach($lists as $key => $text) {
			...
			if(!empty($listStrings)) {
				...
				foreach($listStrings as $str) {
					...
					if(!empty($image)) {
						$arrayString[] = array('type' => 'IMAGE', 'src' => $image);
					} else {
						$arrayString[] = array('type' => 'TEXT', 'text' => $str . ' ');
					}
				}
				$array[] = $arrayString;
			}
	...
	function exportStorages($filename, $storages) {
		...
		for($i=0, $n = count($storages); $i < $n; $i++) {
			...
			foreach($list_answer as $idx => $answer) {
				...
				$template = array('col1' => chr($c++), 'col2' => $answer);
				pr($template);
				$this->_addDataToRow($table, $template, $styleCell);
			}
	...
	private function _addResultStudentToRow($table, $text, $isTitle = false, $styleText = array()) {
		...
		$table->addRow();
		if($isTitle) {
			$styleCell = array('borderSize'=> 6, 'borderColor'=> '006699', 'bgColor' => '66BBFF');
			// Add Cell
			$table->addCell(9000, $styleCell)->addText($text, array('bold' => true, 'color' => 'FFFFFF'));
		} else {
			// Add Cell
	...
	private function _writeAnswerOfStudentDetail($section, $studentAnswerList, $topicDetails) {
		...
		foreach($topicDetails as $key => $value) {
			...
			// write Question
			$this->_writeQuestionOrAnswer($statusCell, ($key+1) . '. ' . $value['question_name'], array('bold' => true, 'italic' => true, 'size' => 16));
			$statusCell->addTextBreak();
			
			if(!isset($answer_of_student['answer'])) {
				$this->_writeQuestionOrAnswer($statusCell, '__ (Không có câu trả lời) __', array('italic' => true, 'size' => 10, 'color' => 'FF0000'));
				$statusCell->addTextBreak();
			}
			...
			foreach($answers as $k => $v) {
				...
				// write Answer
				$this->_writeQuestionOrAnswer($statusCell, chr($number++) . '. ' . $v . ' ' . $correct, array('size' => 12));
			}
			// <br>
			$statusCell->addTextBreak();
		}
